use sorayunara::lexer::{TokenKind, tokenize};

#[test]
fn test_tokenize_primitives() {
    let source =
        "let x: Int = 42 let y: Float = 3.14 let s: String = \"aether\" let b: Bool = true";
    let tokens = tokenize(source).expect("Tokenization should succeed");

    assert!(
        tokens
            .iter()
            .any(|t| matches!(t.kind, TokenKind::IntLit(42)))
    );
    assert!(
        tokens
            .iter()
            .any(|t| matches!(t.kind, TokenKind::StrLit(ref s) if s == "aether"))
    );
